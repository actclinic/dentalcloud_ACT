...
MLS COSTS:
- The MLS tab (Material, Lab & Special Cost) is payment-based. It keeps material, lab, and special doctor items/totals separate on each payment record, alongside collected amount, doctor earned amount, and net profit. Historical treatment-linked costs remain readable for compatibility, and legacy cost rows without a type are treated as material.
...
- Saving payment MLS costs synchronizes the corresponding Material Cost, Lab Cost, or Special Doctor Cost expense records on that payment date and refreshes payment-based doctor commission reporting. All three categories are deducted from profitability and the percentage-commission base for that payment. Do not count the same cost twice or call Patient Balance an MLS cost.
