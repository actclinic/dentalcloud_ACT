...
export interface MedicineSale {
  ...
  unit_price: number;
  total_price: number;
  standard_total_price?: number;   // full price (unit_price × quantity) before any discount
  discount_amount?: number;         // amount discounted (0 = no discount, = standard_total_price → FOC)
