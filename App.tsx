...
const PATIENTS_BACKGROUND_BATCH_SIZE = 1000;
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (locId) {
        ...
        if (!isDoctorSession) {
          ...
          void (async () => {
            try {
              for (let offset = PATIENTS_INITIAL_LOAD_SIZE; ; offset += PATIENTS_BACKGROUND_BATCH_SIZE) {
                if (requestId !== initialDataFetchRequestRef.current) return;
                const batch = await api.patients.getPage(locId, offset, PATIENTS_BACKGROUND_BATCH_SIZE);
                if (requestId !== initialDataFetchRequestRef.current) return;
                const done = batch.length < PATIENTS_BACKGROUND_BATCH_SIZE;
                setPatients((previous) => mergePatientsById(previous, batch));
                setDashboardPatients((previous) => mergePatientsById(previous, batch));
                if (done) break;
              }
            } catch (backgroundError) {
              console.warn('Background patient loading failed:', backgroundError);
            } finally {
        ...
        const [aptData, docData, typeData, recordsData, medData, paymentsData, rescheduleLogsData] = await supportingDataPromise;
        if (requestId !== initialDataFetchRequestRef.current) return;
        ...
        if (isDoctorSession) setPatients(scopedPatients);
  ...
  const handlePaymentVoided = async ({ paymentId, patientId, newBalance }: { paymentId: string; patientId: string; newBalance: number }) => {
    const removePayment = (items: PaymentRecord[]) => items.filter((payment) => payment.id !== paymentId);
    setPaymentRecords(removePayment);
    setPatientPaymentRecords(removePayment);
    setDashboardPayments(removePayment);
    setAssistantPaymentRecords(removePayment);
  ...
  const handleUndoTreatment = async (record: ClinicalRecord) => {
    ...
    try {
      ...
      setSelectedPatient({ ...selectedPatient, balance: res.new_balance });
      setTreatmentHistory((current) => current.filter((treatment) => treatment.id !== record.id));
      setLatestTreatmentBatch((current) => current.filter((treatment) => treatment.id !== record.id));
      setPaymentDraft((current) => ({
        ...current,
        treatments: current.treatments.filter((treatment) => treatment.id !== record.id)
      }));
      setSelectedTreatmentsForReceipt((current) => current.filter((treatment) => treatment.id !== record.id));
    } catch (err: any) {
      alert(err.message);
    }
  };
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.finance.processPayment({
        ...
        staffSessionToken: session?.staffAuthToken || null
      });
      let paymentRecord: PaymentRecord = {
        ...res.payment,
        patient_name: res.payment.patient_name || selectedPatient.name
      };
      const builtPaymentSnapshot = buildPaymentReceiptSnapshot({
        patient: selectedPatient,
