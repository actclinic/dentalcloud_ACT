...
BEGIN
  ...
  INSERT INTO payments (
    location_id, patient_id, amount, original_amount, cleared_amount, remaining_balance,
    payment_method, payment_status, treatment_ids, payment_date,
    created_by_user_id, created_by_user_name
  ) VALUES (
    v_patient.location_id, v_patient.id, v_amount, v_amount, v_amount, COALESCE(v_patient.balance, 0),
    v_method, CASE WHEN COALESCE(v_patient.balance, 0) = 0 THEN 'FULL' ELSE 'PARTIAL' END,
    COALESCE(p_treatment_ids, '{}'), COALESCE(p_payment_date, CURRENT_DATE),
